...
User.get_all_users = function() {
    ...
    this.$http.get(url).then((resp) => {
        if (resp.data.stat == true) {
            ...
            self.users = resp.data.data.users;
            self.role_meta = window.role_meta;
...
User.edit_details = function(user) {
    var user_role = _.find(user.roles, {
        name: "educator"
    });
    if (user_role) {
...
new J.Vue({
    ...
    mounted: function() {
        this.get_all_users();
        this.searchCollection = this.users;
    },
    computed: {
        all_user:function(){
        	return this.users;
